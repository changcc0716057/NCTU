...
chatroom_list = {} # index : chatroom_name(username), chatroom_list[index][0] =port, chatroom_list[index][1] = status
...
def whoami(client_command, client_fd, client_addr, return_message):
    isNumber = True
    try:
        num = int(client_command[1])
    except ValueError:
        isNumber = False
    if len(client_command) != 2 or (not isNumber): #格式不符
        return_message = "Usage: whoami\n% "
    else:
        if login_user_number[num] == None : # 如果用戶傳來的 token number 沒有對到任何 socket，代表尚未 login
            return_message = "Please login first.\n% "
        else :
            return_message = str(login_user_socket[login_user_number[num]]) + "\n% "
        client_fd.sendto(return_message.encode(), client_addr)
...
def logout(client_command, client_fd, return_message):
    name = login_user_socket[client_fd]
    if len(client_command) != 1: # 格式不符
        return_message = "Usage: logout\n% "
    else:
        if client_fd in login_user_socket: # 代表使用這個 socket 的用戶已經 login
            if name in chatroom_list and chatroom_list[name][1] == "open":
                return_message = "Please do “attach” and “leave-chatroom” first.\n% "
            else:
                return_message = "Bye, "+ name +".\n% "
...
def create_chatroom(client_command, client_fd, return_message):
    valid = True

    if len(client_command) == 2:
        try:
            port_num = int(client_command[1])
        except ValueError:
            valid = False
    else:
        valid = False
    ...
    if not valid : #格式不符
        return_message = "Usage: create-chatroom <port>\n% "
    else:
        if client_fd in login_user_socket: # 代表使用這個 socket 的用戶已經 login
            username = login_user_socket[client_fd]
            if username in chatroom_list:
                return_message = "User has already created the chatroom.\n% "
            else:
                chatroom_list[username] = [port_num, "open"]
                return_message = "start to create chatroom...\n"
        else:
            return_message = "Please login first.\n% "
    client_fd.sendall(return_message.encode())

def list_chatroom(client_command, client_fd, client_addr, return_message):
    if len(client_command) != 2: # 格式不符
        return_message = "Usage: list-chatroom\n% "
    else:
        if login_user_number[int(client_command[1])] == None : # 如果用戶傳來的 token number 沒有對到任何 socket，代表尚未 login
            return_message = "Please login first.\n% "
        else :
            maxlen = 13
            for name in chatroom_list: # 為了對齊 找出 chatroom name 中最長的字串
                maxlen = max(maxlen, len(name))
            return_message = "Chatroom_name" + " " * (maxlen - 13 + 3) + "Status"
            for name in chatroom_list:
                return_message += "\n" + str(name) + " " * (maxlen - len(str(name)) + 3) + str(chatroom_list[name][1])
            return_message += "\n% " 
    client_fd.sendto(return_message.encode(), client_addr)

def join_chatroom(client_command, client_fd, return_message):
    if len(client_command) != 2 : # 格式不符
        return_message = "Usage: join-chatroom <chatroom_name>\n% "
    else :
        if client_fd in login_user_socket: # 代表使用這個 socket 的用戶已經 login
            chatroom_name = client_command[1]
            if chatroom_name in chatroom_list and chatroom_list[chatroom_name][1] == "open":
                return_message = str(chatroom_list[chatroom_name][0])
            else:
                return_message = "The chatroom does not exist or the chatroom is close.\n% "
        else :
            return_message = "Please login first.\n% "
    client_fd.sendall(return_message.encode())  

def restart_chatroom(client_command, client_fd, return_message):
    if len(client_command) != 1: #格式不符
        return_message = "Usage: restart-chatroom\n% "
    else:
        if client_fd in login_user_socket: # 代表使用這個 socket 的用戶已經 login
            chatroom_name = login_user_socket[client_fd]
            if chatroom_name in chatroom_list:
                if chatroom_list[chatroom_name][1] == "close":
                    return_message = "start to create chatroom...\n" + str(chatroom_list[chatroom_name][0])
                    chatroom_list[chatroom_name][1] = "open"
                else:
                    return_message = "Your chatroom is still running.\n% "
            else:
                return_message = "Please create-chatroom first.\n% "
        else:
            return_message = "Please login first.\n% "
    client_fd.sendall(return_message.encode())
...
def TCP_thread_fun(conn):
    ...
    while True:
        ...
        elif client_command[0] == "create-chatroom" :
            create_chatroom(client_command, conn, return_message)
        elif client_command[0] == "join-chatroom" :
            join_chatroom(client_command, conn, return_message)
        elif client_command[0] == "restart-chatroom" :
            restart_chatroom(client_command, conn, return_message)
        elif client_command[0] == "close" :
            chatroom_list[client_command[1]][1] = "close"
        else :
...
def UDP_thread_fun(UDP_fd):
    while True:
        ...
        elif client_command[0] == "list-chatroom" :
            list_chatroom(client_command, UDP_fd, client_addr, return_message)
        function_Lock.release()
